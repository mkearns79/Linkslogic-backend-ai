...
class SimplifiedGolfRulesSystem:
    ...
    def _check_template_strict(self, question: str, verbose: bool = False) -> Optional[Dict]:
        ...
        template_patterns = {
            'clear_lost_ball': {
                # FIX 7: Relaxed â€” only 'ball' required; 'lost' moved to any_of with synonyms
                'required': ['ball'],
            ...
            'water_hazard_16': {
                ...
                # FIX 1: Exclude â€” if query is about a non-water topic, let AI handle it
        ...
        for template_name, patterns in template_patterns.items():
            ...
            # FIX 1 (revised): Exclusion check â€” if query contains words indicating
    ...
    def _is_definition_query(self, question: str) -> bool:
        ...
        # FIX 4: Negative patterns â€” these are procedural, not definitional
    ...
    def _get_unified_ai_response(self, question: str, verbose: bool = False, query_id: str = "") -> Dict:
        ...
        try:
            ...
            if not filtered_results:
                ...
                if verbose:
                    logger.info(f"âš ï¸ No rules scored >0.5, using top {len(filtered_results)} as fallback")
            ...
            import traceback
            logger.error(f"âŒ [{query_id}] Unified AI error: {e}\n{traceback.format_exc()}") 
            return {
    ...
    def _enrich_ai_response(self, ai_answer: str, question: str) -> str:
        ...
        enrichment_triggers = [
            {
                ...
                'header': "\n\n---\n**Columbia CC Local Rule â€” Hole 16 Penalty Area Options:**\n\n"
            },
            {
                'template_name': 'water_hazard_17',
                'condition': ('penalty area' in answer_lower and '17' in question_lower),
                'header': "\n\n---\n**Columbia CC Local Rule â€” Hole 17 Penalty Area Options:**\n\n"
            },
            {
                ...
                'header': "\n\n---\n**Columbia CC Local Rule â€” Lost Ball Alternative:**\n\n"
            },
            {
                ...
                'header': "\n\n---\n**Columbia CC Local Rule â€” Out of Bounds Alternative:**\n\n"
            },
            {
                ...
                'header': "\n\n---\n**Columbia CC Local Rule â€” Maintenance Facility:**\n\n"
            },
        ]
        ...
        for trigger in enrichment_triggers:
            if trigger['condition']:
                ...
                if template:
                    logger.info(f"ðŸ“Ž Enriching AI response with template: {trigger['template_name']}")
            ...
            if 'conditions' in rule:
                try:
                    ...
                    if not isinstance(conditions_list, list):
                        logger.error(f"âš ï¸ Conditions for {rule_id} is not a list: {type(conditions_list)}")
                    ...
                    for condition in conditions_list[:5]:
                        ...
                        if not isinstance(condition, dict):
                            logger.error(f"âš ï¸ Condition in {rule_id} is {type(condition)}, not dict")
                            continue
                    ...
                    logger.error(f"âš ï¸ Error formatting conditions for rule {rule_id}: {e}")
            ...
            for rule_id in list(related_rules_to_add)[:3]:  # Include up to 3 related rules
                ...
                if rule:
                    try:
                        ...
                        if 'conditions' in rule:
                            ...
                            if isinstance(conditions_list, list):
                                ...
                                if exceptions:
                                    related_part += "\nâš ï¸ EXCEPTIONS:\n"
                                    for exc in exceptions:
                                        related_part += f"  â€¢ {exc.get('explanation', '')}\n"
                                        if 'examples' in exc and isinstance(exc.get('examples'), list):
                                            for ex in exc['examples'][:2]:
                        ...
                        logger.error(f"âš ï¸ Error formatting related rule {rule_id}: {e}")
...
2. CHECK FOR EXCEPTIONS - This is absolutely critical! Consider:
   ...
   - WHEN it happened:
     ...
     * IMPORTANT: If ball was lifted and replaced BEFORE natural forces moved it â†’ Rule 9.3 Exception applies (replace ball, not play as it lies)
   
   - WHERE on the course:
     * Putting green â†’ Special rules under Rule 13
     * Penalty area â†’ Rule 17 procedures
     * Bunker â†’ Rule 12 specific rules
     * Teeing area â†’ Rule 6 applies (including Rule 6.2b(5): no penalty for accidentally moving ball on tee)
   ...
   - INTENT (accidental vs. deliberate):
     ...
     * IMPORTANT: Accidental contact during backswing or practice swing is NOT a stroke â€” a stroke requires intent to hit the ball forward. If the ball was accidentally knocked off the tee during a backswing, it is NOT a stroke and Rule 6.2b(5) applies.
...
3. CHECK COLUMBIA CC LOCAL RULES:
   ...
   - IMPORTANT: Only reference Columbia CC local rules that are explicitly provided in the context above. Do NOT suggest that Columbia CC "may have" a local rule or recommend checking with the pro shop about local rules that might exist. If no local rule is provided in the context, give only the official USGA ruling.
    ...
    def _log_query_complete(self, query_id: str, question: str, result: Dict):
        ...
        try:
            ...
            if result.get('source') == 'error':
                logger.error(f"âŒ [{query_id}] Query failed: {result.get('error', 'Unknown error')}")
            else:
